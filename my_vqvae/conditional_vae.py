...
class Decoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, condition_im: dict=None) -> torch.Tensor:
        for idx, block in enumerate(self.blocks):
            x = block(x)
            if idx + 2 < len(self.blocks) and self.blocks[idx+2].__class__.__name__ in ['Upsample', 'GroupNorm']:
                cache_im = condition_im[str(tuple(x.shape))]
                x = x + cache_im
